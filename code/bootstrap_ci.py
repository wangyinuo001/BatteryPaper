...
def load_discharge_cycle(mat_path, cycle_idx=0):
    ...
    try:
        ...
        return {'time_s': time_s, 'voltage': voltage_d, 'current': current_d,
                'soc': soc, 'Q_total': Q_total}
    except Exception:
        return None
...
def compute_tte_with_perturbation(scenario, temperature_c=25,
                                   Q0_scale=1.0, R0_scale=1.0, param_noise=0.0):
...
def main():
    ...
    for scenario_name, scenario in SCENARIOS.items():
        ...
        print(f"  {scenario_name:12s}: TTE = {tte_est:.3f} [{tte_lo:.3f}, {tte_hi:.3f}] h")
    ...
    if batteries and b2_batteries:
        def rmse_gap_stat(combined):
            ...
            rmse1 = np.mean([
                np.sqrt(np.mean((shepherd_voltage(d['soc'], d['current'], d['Q_total']) - d['voltage'])**2)) * 1000
                for d in b1_sample
            ])
            rmse2 = np.mean([
                np.sqrt(np.mean((shepherd_voltage(d['soc'], d['current'], d['Q_total']) - d['voltage'])**2)) * 1000
        ...
        gap_est, gap_lo, gap_hi = bootstrap_ci(combined, rmse_gap_stat, n_bootstrap=n_boot)
        ci_rows.append({
    ...
    with open(tex_path, 'w', encoding='utf-8') as f:
        ...
        f.write('\\centering\n')
        f.write('\\caption{95\\% bootstrap confidence intervals for key metrics '
                '($n_{\\text{boot}}=2000$)}\n')
        ...
        for _, row in df.iterrows():
            f.write(f"{row['Metric']} & {row['Estimate']} & "
