...
def main():
    ...
    if batch2_data:
        ...
        print(f"  Batch-2 fitted params: E0={b2_params['E0']:.4f}, K={b2_params['K']:.4f}, "
    ...
    print(f"\nSame-batch (B1→B1):  RMSE = {b1_rmse_mean:.2f} ± {b1_rmse_std:.2f} mV "
          f"(n={len(b1_metrics)})")
    print(f"Cross-batch (B1→B2): RMSE = {b2_rmse_mean:.2f} ± {b2_rmse_std:.2f} mV "
          f"(n={len(b2_metrics)})")
    ...
    summary_rows = [
        {'Evaluation': 'Same-batch (B1→B1)', 'N': len(b1_metrics),
        ...
        {'Evaluation': 'Cross-batch (B1→B2)', 'N': len(b2_metrics),
    ...
    with open(tex_path, 'w', encoding='utf-8') as f:
        ...
        f.write('\\centering\n')
        f.write('\\caption{Cross-batch validation: Shepherd model fitted on XJTU Batch-1 '
        ...
        for _, row in summary_df.iterrows():
            f.write(f"{row['Evaluation']} & {row['N']} & {row['RMSE (mV)']} & "
                    f"{row['MAE (mV)']} \\\\\n")
        f.write('\\hline\n')
        f.write('\\end{tabular}\n')
        f.write('\\end{table}\n')
    ...
    if batch1_data:
        ...
        d = batch1_data[0]
        v_pred = shepherd_voltage(d['soc'], d['current'], d['Q_total'], SHEP_BATCH1)
        ax.plot(d['time_s']/60, d['voltage'], 'k-', lw=2, label='Experimental', alpha=0.8)
    ...
    if batch2_data:
        ...
        if batch2_data and 'b2_params' in dir():
            ...
            ax.plot(d['time_s']/60, v_refit, 'b:', lw=1.5, label='Shepherd (Batch-2 refit)')
        ax.set_xlabel('Time (min)')
        ax.set_ylabel('Voltage (V)')
    ...
    bp = ax.boxplot(box_data, labels=['Same-batch\n(B1→B1)', 'Cross-batch\n(B1→B2)'],
                    patch_artist=True)
    ...
    plt.suptitle('Cross-Batch Validation: Shepherd Model Generalizability',
