...
from thevenin_ecm import (
    RintModel, TheveninFirstOrder, TheveninSecondOrder,
    identify_thevenin_params, fit_ocv_soc, ocv_polynomial
)
...
def run_pointwise_comparison(data, ocv_coeffs, thev1_params, thev2_params):
    ...
    predictions = {
        'NBM': v_nbm, 'Shepherd (Proposed)': v_shep,
        'Rint': v_rint, 'Thevenin-1RC': v_thev1, 'Thevenin-2RC': v_thev2,
    }
...
def main():
    ...
    for i in range(1, 9):
        ...
        if os.path.exists(mat_path):
            ...
            if data is not None:
                ...
                print(f"Loaded Batch-1/2C_battery-{i}: Q={data['Q_total']:.3f}Ah, "
    ...
    print(f"  R0={thev1_params['R0']:.4f}, R1={thev1_params['R1']:.4f}, "
    ...
    print(f"  R0={thev2_params['R0']:.4f}, R1={thev2_params['R1']:.4f}, "
    ...
    for bat in batteries:
        ...
        metrics, preds = run_pointwise_comparison(bat, ocv_coeffs, thev1_params, thev2_params)
        for model_name, m in metrics.items():
            row = {'Battery': bat['name'], 'Model': model_name}
            row.update(m)
            all_rows.append(row)
            print(f"  {model_name:20s}: RMSE={m['RMSE (mV)']:7.2f}mV  "
    ...
    for model in ['NBM', 'Rint', 'Thevenin-1RC', 'Thevenin-2RC', 'Shepherd (Proposed)']:
        if model in agg.index:
            ...
            print(f"  {model:20s}: RMSE={rmse_m:6.2f}±{rmse_s:.2f}  "
    ...
    with open(tex_path, 'w', encoding='utf-8') as f:
        ...
        f.write('\\centering\n')
        f.write('\\caption{Voltage prediction accuracy comparison across baseline models '
                '(averaged over 8 XJTU Batch-1 batteries, Cycle 0)}\n')
        f.write('\\label{tab:baseline_comparison}\n')
        f.write('\\begin{tabular}{lcccc}\n')
        f.write('\\hline\n')
        f.write('Model & RMSE (mV) & MAE (mV) & Max Error (mV) & Rel. Error (\\%) \\\\\n')
        f.write('\\hline\n')
        for _, row in summary_df.iterrows():
            f.write(f"{row['Model']} & {row['RMSE (mV)']} & {row['MAE (mV)']} & "
    ...
    colors = {'NBM': '#3b82f6', 'Rint': '#f59e0b', 'Thevenin-1RC': '#10b981',
              'Thevenin-2RC': '#8b5cf6', 'Shepherd (Proposed)': '#ef4444'}

    ax1 = axes[0]
    ax1.plot(bat['time_s']/3600, bat['voltage'], 'k-', lw=2, label='Experimental', alpha=0.8)
    ...
    plt.suptitle('Baseline Comparison: Voltage Accuracy on XJTU Batch-1', fontweight='bold')
    plt.tight_layout()
